...
public class Common_Functions {
	...
	public void startRecording() throws Exception {
		...
		this.screenRecorder = new ScreenRecorder(gc, gc.getBounds(),
				...
				null, new File(Constants.videoFolder));
		// new Format(MediaTypeKey, MediaType.VIDEO, EncodingKey, "black",
		// FrameRateKey, Rational.valueOf(30)), null,new File(targetFolder));
				...
				if ("TABLE".equalsIgnoreCase(element.getElementType().name())) {
					...
					for (XWPFTable table : tableList) {
						...
						for (int j = 0; j < noCols; j++) {
							...
							/*
							 * if (table.getRow(0).getCell(j).getText().trim().
							 * equalsIgnoreCase("Second")) {
							 * System.out.println(table.getRow(j).getCell(1).
							 * getText());
							 * 
							 * }
							 */
						...
						for (Map.Entry m : map.entrySet()) {
							// System.out.println("out key="+m.getKey()+" - out
							// value="+m.getValue());
						}
